...
async def add_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_owner(update.effective_user.id):
        return await not_authorized(update)
    if not context.args:
        return await update.effective_message.reply_text("Usage: /addadmin <user_id>")
...
async def del_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_owner(update.effective_user.id):
        return await not_authorized(update)
    if not context.args:
        return await update.effective_message.reply_text("Usage: /removeadmin <user_id>")
    uid = int(context.args[0])
...
async def ban_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_owner(update.effective_user.id):
        return await not_authorized(update)
    if not context.args:
        return await update.effective_message.reply_text("Usage: /ban <user_id> [reason]")
    uid = int(context.args[0])
    reason = " ".join(context.args[1:]) if len(context.args) > 1 else None
    await set_banned(uid, True, reason)
    await update.effective_message.reply_text(
        f"🚫 Banned user `{uid}`. {('Reason: '+reason) if reason else ''}",
        parse_mode=ParseMode.MARKDOWN
    )
...
async def setreaction_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = update.effective_user.id
    if not (is_owner(uid) or await is_admin(uid)):
        return await not_authorized(update)
    if not context.args:
...
async def setpromotion_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_owner(update.effective_user.id):
        return await not_authorized(update)
    if not context.args:
        return await update.effective_message.reply_text("Usage: /setpromotion <t.me link | @username | url>")
...
async def send_chat_page(chat_id: int, context: ContextTypes.DEFAULT_TYPE, page: int):
    ...
    if not items:
        await context.bot.send_message(chat_id, "No chats.")
        return
...
async def block_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_owner(update.effective_user.id):
        return await not_authorized(update)
    if not context.args:
        return await update.effective_message.reply_text("Usage: /block <chat_id>")
...
async def unblock_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_owner(update.effective_user.id):
        return await not_authorized(update)
    if not context.args:
        return await update.effective_message.reply_text("Usage: /unblock <chat_id>")
    cid = int(context.args[0])
...
async def leave_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_owner(update.effective_user.id):
        return await not_authorized(update)
    if not context.args:
        return await update.effective_message.reply_text("Usage: /leave <chat_id>")
    cid = int(context.args[0])
    try:
...
async def menu_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("menu:list:"):
        if not await ensure_auth(need_owner=True):
            return
        page = int(data.split(":")[-1])
        await q.answer()
        await send_chat_page(q.message.chat_id, context, max(1, page))
        return

    if data == "menu:stats":
        if not await ensure_auth(need_admin=True):
            return
        await q.answer()
    ...
    if data == "menu:setreaction_help":
        if not await ensure_auth(need_owner=True):
            return
        current = await get_reaction_emoji()
        await q.edit_message_text(f"Set reaction:\n`/setreaction <emoji>`\nCurrent: {current}", parse_mode=ParseMode.MARKDOWN)
        return
